  ...
  if [[ "${KEY,,}" == *"$MEDIA_FORMAT"* ]] > /dev/null 2>&1; then
    ...
    if [ "$CUSTOM_VOCABULARY" == "" ] > /dev/null 2>&1; then
      JSON=$(jq  -n \
              ...
              --arg key3    "$FILE_URL"              \
              '{TranscriptionJobName: $key0, LanguageCode: $key1, MediaFormat: $key2, Media: {MediaFileUri: $key3}, OutputBucketName: "ceo-transcripts", Settings: { ShowSpeakerLabels:true, MaxSpeakerLabels:4}}')
    else
      JSON=$(jq  -n \
              --arg key0    "$NAME"                  \
              --arg key1    "$LANGUAGE_CODE"         \
              ...
              '{TranscriptionJobName: $key0, LanguageCode: $key1, MediaFormat: $key2, Media: {MediaFileUri: $key3}, OutputBucketName: "ceo-transcripts", Settings: {VocabularyName: $key4, ShowSpeakerLabels:true, MaxSpeakerLabels:4}}')
    fi    
